...
@section('content')
    <div class="container col-lg-8" style="margin-top: 30px; margin-bottom: 5px">
        <form action="{{ url('dashboard/courses/' . $teacherCourse->name . '/update') }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="inputCourseName"
                    placeholder="ex. Data Science" value="{{ old('name', $teacherCourse->name) }}">
                @error('name')
            ...
            <div class="form-group" style="margin-top: 10px;">
                ...
                <textarea rows="5" id="summernote" name="description"
                    class="form-control @error('description') is-invalid @enderror">{{ old('description', $teacherCourse->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
