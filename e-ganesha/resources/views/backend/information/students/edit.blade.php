...
@section('breadcrumbs')
    {{ Breadcrumbs::render('student_edit', $student) }}
@endsection

@section('content')
    <div class="col-lg-13 my-3">
        <form action="{{ route('students.update', $student->name) }}" method="POST">
            @method('PUT')
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="inputClassYear" class="form-label">Kelas</label>
                <select name="class_year_id" class="form-select @error('class_year_id') is-invalid @enderror">
                    ...
                    @foreach ($classYears as $classYear)
                        @if (old('class_year_id', $student->class_year_id) == $classYear->id)
